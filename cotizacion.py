...
def cotizacion_funcion(llave_1:str, llave_2:str,llave_3:str,llave_4:str):
    ...
    try:
        ...
        with open(RUTA_PRODUCTOS, 'r', encoding='utf-8') as file:
            ...
            with col1:
                calcular_total = st.button('Calcular Total',width='stretch',key=llave_2)
            with col2:
                limpiar_tabla = st.button('Limpiar - Cargar Cache',width='stretch',key=llave_3)
            with col3:
                imprimir = st.button('Imprimir', type='primary', width='stretch',key=llave_4)
            ...
            if imprimir:
                ...
                st.info('Ya Se Puede Limpiar La Tabla')
...
def cotizacion_por_codigo():
    ...
    try:
        ...
        with open(RUTA_PRODUCTOS, 'r', encoding='utf-8') as file:
            ...
            # Asigno el DataFrame vacio y los contadores al cache de streamlit
            if 'df_cod' not in st.session_state:
                st.session_state.df_cod = DF_VACIO
            if 'total_cod' not in st.session_state:
                st.session_state.total_cod = cotizacion_total
            if 'productos_cod' not in st.session_state:
                st.session_state.productos_cod = productos_en_cotizacion
            
            # Creo un DataFrame editable con filas dinamicas, asigno la salida a "edicion"
            edicion = st.data_editor(
                data=st.session_state.df_cod,
                key='edicion_1',
                num_rows='dynamic',
                column_order=['Unidades','Codigo','Productos','Precio U.','Total','Existencias'],
                hide_index=True,
                column_config={
                    'Unidades':st.column_config.NumberColumn(disabled=False,width=80,help=':orange[Unidades]',step=1),
                    'Codigo':st.column_config.SelectboxColumn(options=df['Codigo'].to_list(),width=60,help=':orange[Escanea el Codigo]',required=True),
                    'Productos':st.column_config.TextColumn(disabled=True,width=300,help=':orange[Producto Y Modelo]'),
                    'Precio U.':st.column_config.NumberColumn(format='dollar', disabled=True, width=60, help=':orange[Precio Unitario]'),
                    'Total':st.column_config.NumberColumn(disabled=True,width=60, format='dollar',help=':orange[Total]'),
                    'Existencias':st.column_config.TextColumn(disabled=True, width=60,help=':orange[Existencias]')
                }
            )
            ...
            # Inicializo los widgets para los contadores en la UI
            metric1,metric2 = st.columns([2,2])
            metric1.metric('Productos en la Cotizacion',value=st.session_state.productos_cod,border=True)
            metric2.metric('Precio Total Cotizacion',value=st.session_state.total_cod,border=True)
            ...
            col1, col2, col3 = st.columns(3)
            with col1:
                calcular_total = st.button('Calcular Total',width='stretch',key='calculo_cod')
            with col2:
                limpiar_tabla = st.button('Limpiar - Cargar Cache',width='stretch',key='limpiar_cod')
            with col3:
                imprimir = st.button('Imprimir', type='primary', width='stretch',key='imprimir_cod')
            ...
            # Uno la salida "edicion" con "df" con sus indices correspondientes "Producto"
            if calcular_total or imprimir:
                edicion = edicion.dropna(axis=0,how='any',subset=['Unidades','Codigo'])
                edicion = edicion.drop_duplicates(subset='Codigo')
                if edicion.empty:
                    st.session_state.df_cod = DF_VACIO
                else:
                    filtro = edicion['Codigo'].tolist()
                    ...
                    union['Total'] = union['Unidades'] * union['Precio Lista']
                    union['Existencias'] = union['Cantidad']
                    union['Productos'] = union['Producto']

                    union = union.reindex(columns=['Unidades','Codigo','Productos','Precio U.','Total','Existencias'])
            ...
            if imprimir:
                ...
                # Itero sobre los datos que estan en la tabla final guardada en el cache de streamlit
                # y los asigno a su celda pdf.
                for index, row in st.session_state.df_cod.iterrows():
                    pdf.cell(16,6,txt=f'{row['Unidades']}',border=True,align='C') 
                    pdf.cell(122,6,txt=f'{row['Productos']}',border=True) 
                    pdf.cell(29,6,txt=f'$ {row['Precio U.']}',border=True,align='R') 
                    pdf.cell(29,6,txt=f'$ {row['Total']}', ln=True,border=True,align='R')
                ...
                # Determino formatos e imprimo los contadores al final de la tabla.
                pdf.cell(0,2,fill=True,ln=True)
                pdf.cell(half_page,6,txt=f'Total De Productos: {st.session_state.productos_cod}', border=True, align='C')
                pdf.cell(half_page,6,txt=f'Total De Cotizacion: $ {st.session_state.total_cod}', border=True,align='C',ln=True)
...
tab_1, tab_2 = st.tabs(['Por Nombre','Por Codigo'])
